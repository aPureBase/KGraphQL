...
interface FunctionWrapper<T> : Publisher {
    ...
    companion object {
        fun <T> on(function: KFunction<T>): FunctionWrapper<T> = ArityN(function)

        fun <T> on(function: suspend () -> T): FunctionWrapper<T> = ArityZero(function)

        fun <T, R> on(function: suspend (R) -> T) = ArityOne(function, false)

        fun <T, R> on(function: suspend (R) -> T, hasReceiver: Boolean = false) = ArityOne(function, hasReceiver)

        fun <T, R, E> on(function: suspend (R, E) -> T, hasReceiver: Boolean = false) = ArityTwo(function, hasReceiver)
        ...
        fun <T, R, E, W, Q, A, S, G, H> on(
            function: suspend (R, E, W, Q, A, S, G, H) -> T,
    ...
    class AritySix<T, R, E, W, Q, A, S>(
        ...
        override suspend fun invoke(vararg args: Any?): T? {
            if (args.size == arity()) {
                val t =
                    implementation(args[0] as R, args[1] as E, args[2] as W, args[3] as Q, args[4] as A, args[5] as S)
                subscribers.forEach {
                    it.value?.onNext(t)
                }
                return t
